...
const orderService = new go_service_1.OrderService();
const ratingService = new go_service_1.RatingService();
...
router.post('/estimate', async (req, res, next) => {
    try {
        const { type, pickupLatitude, pickupLongitude, pickupAddress, dropoffLatitude, dropoffLongitude, dropoffAddress, vehicleType, packageSize } = req.body;
        if (!type || pickupLatitude === undefined || pickupLongitude === undefined ||
            dropoffLatitude === undefined || dropoffLongitude === undefined) {
            throw new core_1.AppError('type, pickup/dropoff coordinates are required', core_1.ErrorCode.INVALID_INPUT, 400);
        }
...
router.get('/history', async (req, res, next) => {
    try {
        ...
        if (!userIdentifier) {
            throw new core_1.AppError('User identifier missing', core_1.ErrorCode.UNAUTHORIZED, 401);
        }
...
router.patch('/:id/status', async (req, res, next) => {
    try {
        ...
        if (!status) {
            throw new core_1.AppError('status is required', core_1.ErrorCode.INVALID_INPUT, 400);
        }
        const location = latitude && longitude ? { latitude, longitude } : undefined;
        const order = await orderService.updateOrderStatus(req.params.id, status, location);
        res.json({
            success: true,
            data: order,
        });
...
router.post('/:id/rate', async (req, res, next) => {
    try {
        ...
        if (!rating || rating < 1 || rating > 5) {
            throw new core_1.AppError('rating must be between 1 and 5', core_1.ErrorCode.INVALID_INPUT, 400);
        }
        // Get order to find the other user
        const order = await orderService.getActiveOrderForUser(req.user.id);
        if (!order || order.id !== req.params.id) {
            throw new core_1.AppError('Order not found', core_1.ErrorCode.ORDER_NOT_FOUND, 404);
        }
...
router.get('/:id', async (req, res, next) => {
    try {
        ...
        throw new core_1.AppError('Not implemented', core_1.ErrorCode.INTERNAL_ERROR, 501);
    }
    catch (error) {
        next(error);
    }
