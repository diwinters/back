...
exports.OrderService = exports.updateOrderStatusSchema = exports.acceptOrderSchema = exports.createOrderSchema = void 0;
const zod_1 = require("zod");
const core_1 = require("@gominiapp/core");
const driver_service_1 = require("./driver.service");
// Validation schemas
exports.createOrderSchema = zod_1.z.object({
    type: zod_1.z.enum(['RIDE', 'DELIVERY']),
    ...
    pickupName: zod_1.z.string().optional(),
    dropoffLatitude: zod_1.z.number().min(-90).max(90),
    dropoffLongitude: zod_1.z.number().min(-180).max(180),
    dropoffAddress: zod_1.z.string().min(1),
    dropoffName: zod_1.z.string().optional(),
    vehicleType: zod_1.z.string().default('ECONOMY'), // Dynamic - accepts any configured vehicle type
    // Delivery specific
    packageSize: zod_1.z.enum(['SMALL', 'MEDIUM', 'LARGE', 'EXTRA_LARGE']).optional(),
    recipientName: zod_1.z.string().optional(),
    recipientPhone: zod_1.z.string().optional(),
    packageDescription: zod_1.z.string().optional(),
    // Admin assignment
    assignedDriverId: zod_1.z.string().optional(),
});
...
const pendingOrderTimeouts = new Map();
class OrderService {
    driverService;
    constructor() {
     ...
     */
    async detectCity(latitude, longitude) {
        // Use the canonical GeoService for consistent NEAREST city detection
        const result = await core_1.GeoService.detectCity(latitude, longitude, 'rides');
        if (result.city) {
     ...
     */
    async getCityPricing(cityId, vehicleTypeCode) {
        const cityPricing = await core_1.prisma.cityVehiclePricing.findUnique({
            where: {
     ...
     */
    calculateFare(distanceKm, durationMinutes, vehicleType, cityPricing) {
        if (cityPricing) {
            ...
            const fare = Math.max(config.baseFare + (distanceKm * config.perKm) + (durationMinutes * config.perMin), config.minimum);
     ...
     */
    startOrderTimeout(orderId, order, excludedDriverIds = []) {
        // Clear any existing timeout
        this.clearOrderTimeout(orderId);
        const timeout = setTimeout(async () => {
            try {
                // Check if order is still pending
                const currentOrder = await core_1.prisma.order.findUnique({
                    where: { id: orderId },
                    include: { user: true },
                });
                if (currentOrder && currentOrder.status === 'PENDING') {
                    core_1.logger.info('Order timed out, re-broadcasting', { orderId, excludedCount: excludedDriverIds.length });
                    // Record timeout event
                    await core_1.prisma.orderEvent.create({
                        data: {
                            orderId,
                            eventType: 'TIMEOUT',
                ...
                core_1.logger.error('Error handling order timeout', { orderId, error });
            }
        }, ORDER_TIMEOUT_MS);
    ...
    async getEstimate(data) {
        const validated = exports.createOrderSchema.parse(data);
        // Detect city from pickup location
        const city = await this.detectCity(validated.pickupLatitude, validated.pickupLongitude);
        if (!city) {
            throw new core_1.AppError('Service is not available in your area', core_1.ErrorCode.SERVICE_UNAVAILABLE, 400);
        }
        // Get city-specific pricing
        const cityPricing = await this.getCityPricing(city.cityId, validated.vehicleType);
        // Calculate distance
        const distanceKm = this.calculateDistance(validated.pickupLatitude, validated.pickupLongitude, validated.dropoffLatitude, validated.dropoffLongitude) / 1000;
        ...
        const nearbyDrivers = await this.driverService.findNearbyDrivers(validated.pickupLatitude, validated.pickupLongitude, { radiusKm: 10, limit: 50 });
        return {
    ...
    async createOrder(userId, data) {
        ...
        const order = await core_1.prisma.order.create({
            data: {
                type: validated.type,
        ...
        if (assignedDriver) {
            ...
            core_1.logger.info('Order created and assigned to driver', { orderId: order.id, driverId: assignedDriver.id });
            const wsServer = (0, core_1.getWebSocketServer)();
            if (wsServer) {
                // Notify Driver
            ...
            core_1.logger.info('Order created', { orderId: order.id, userId, type: validated.type });
            ...
            if (wsServer) {
                ...
                wsServer.broadcastToNearbyDrivers(validated.pickupLatitude, validated.pickupLongitude, 5000, // 5km radius
                {
                    type: 'new_order',
                    payload: orderData,
                }).then(() => {
                    core_1.logger.info('Order broadcast to nearby drivers', { orderId: order.id });
                    // Start timeout for auto-rebroadcast if no driver accepts
                    this.startOrderTimeout(order.id, order);
                }).catch((err) => {
                    core_1.logger.error('Failed to broadcast order to drivers', { orderId: order.id, error: err.message });
                });
            }
        }
        return order;
    ...
    async acceptOrder(driverId, orderId) {
        ...
        if (order.status !== 'PENDING') {
            throw new core_1.ValidationError('Order is no longer available');
        }
        // Clear the timeout since order is being accepted
        this.clearOrderTimeout(orderId);
        const updatedOrder = await core_1.prisma.order.update({
            where: { id: orderId },
            data: {
        ...
        core_1.logger.info('Order accepted', { orderId, driverId });
        // Notify user that driver accepted their order
        const wsServer = (0, core_1.getWebSocketServer)();
        if (wsServer && updatedOrder.user) {
            await wsServer.sendOrderUpdate(updatedOrder.user.did, orderId, 'DRIVER_ASSIGNED', {
                driver: {
            ...
            core_1.logger.info('User notified of driver assignment', { orderId, userDid: updatedOrder.user.did });
        }
        return updatedOrder;
    }
    /**
     ...
     */
    async declineOrder(driverId, orderId) {
        const order = await core_1.prisma.order.findUnique({
            where: { id: orderId },
            include: { user: true },
        });
        ...
        core_1.logger.info('Order declined by driver', { orderId, driverId });
     ...
     */
    async rebroadcastOrder(order, excludeDriverIds) {
        const wsServer = (0, core_1.getWebSocketServer)();
        if (!wsServer)
            return;
        const orderData = {
        ...
        const declinedEvents = await core_1.prisma.orderEvent.findMany({
            where: {
                orderId: order.id,
                eventType: 'DECLINED',
            },
        ...
        await wsServer.broadcastToNearbyDrivers(order.pickupLatitude, order.pickupLongitude, 5000, // 5km radius
        {
            type: 'new_order',
            payload: orderData,
        }, allExcludedDriverIds);
        core_1.logger.info('Order re-broadcast to drivers', { orderId: order.id, excludedCount: allExcludedDriverIds.length });
    }
    /**
     * Update order status
     */
    async updateOrderStatus(orderId, status, location) {
        ...
        core_1.logger.info('Order status updated', { orderId, status });
        ...
        if (wsServer && updatedOrder.user) {
            ...
            await wsServer.sendOrderUpdate(updatedOrder.user.did, orderId, status, {
            ...
            core_1.logger.info('User notified of status update', { orderId, status, userDid: updatedOrder.user.did });
        }
        return updatedOrder;
    }
    /**
    ...
    async cancelOrder(orderId, userId, reason) {
        ...
        if (order.status === 'COMPLETED') {
            throw new core_1.ValidationError('Cannot cancel completed order');
        }
        await core_1.prisma.order.update({
            where: { id: orderId },
            data: {
                status: 'CANCELLED',
        ...
        core_1.logger.info('Order cancelled', { orderId, cancelledBy: userId, reason });
        ...
        if (wsServer) {
            ...
            if (order.driver?.driver) {
                ...
                const driverUser = await core_1.prisma.user.findUnique({
    ...
    async getActiveOrderForUser(userId) {
        return core_1.prisma.order.findFirst({
            where: {
                userId,
                status: {
     ...
     */
    async getOrderHistory(userIdentifier, // can be internal userId (UUID) or DID
    options = {}) {
        ...
        const [orders, total] = await Promise.all([
            ...
            core_1.prisma.order.count({ where: whereClause }),
        ]);
        return { orders, total };
    }
    /**
